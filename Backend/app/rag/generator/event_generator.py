...
class EventGenerator:
    ...
    async def event_generator(self):
        ...
        await self._handle_post_streaming()
    ...
    async def _handle_post_streaming(self):
        ...
        try:
            if self.final_payload:
                assistant_content = (
                    self.final_payload.get("response", "")
                    if isinstance(self.final_payload, dict)
                    else str(self.final_payload)
                )
                ...
                await log_rag_interaction(
                    {
                        "question": self.query,
                        "answer": self.final_payload.get("response", ""),
                        "contexts": contexts,
                        "metadata": {
