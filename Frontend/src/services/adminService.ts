...
export const triggerEmbedding = async () => {
  const response = await axiosInstance.post<{ message: string }>('/api/chunks/embed')
  return response.data
}

export const annotateChunk = async (chunkId: string) => {
  const response = await axiosInstance.post<CodeChunk>(`/annotation/${chunkId}`)
  return response.data
}
...
export const getAnnotationStats = async () => {
  const response = await axiosInstance.get<AnnotationStats>('/api/admin/annotation-stats')
  return response.data
}
...
export const startBatchAnnotation = async (limit?: number) => {
  ...
  const response = await axiosInstance.post('/annotation/batch/unannotated', null, { params })
  return response.data
}
