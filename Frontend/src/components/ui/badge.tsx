...
const Badge = ({ children, variant = "default", className = "" }: { children: React.ReactNode, variant?: "default" | "secondary" | "destructive" | "outline", className?: string }) => {
  ...
  return <div className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors dark:border-zinc-800 ${variants[variant]} ${className}`}>{children}</div>
}
