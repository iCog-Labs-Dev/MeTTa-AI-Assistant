  ...
  return (
    <>
      ...
      >
        {/* Collapsed View */}
        <div
          className={`flex flex-col items-center py-3 gap-2 ${isCollapsed ? "flex" : "hidden"}`}
        >
          <button
        ...
        <div
          className={`flex flex-col flex-1 overflow-hidden transition-opacity ${isCollapsed ? "duration-100 opacity-0 hidden" : "duration-150 opacity-100 delay-150"}`}
        >
      ...
      <SearchModal
        ...
        items={sessionItems}
        filterFn={(sessionItem, query) =>
          sessionItem.title.toLowerCase().includes(query.toLowerCase())
        }
        renderItem={(sessionItem, isActive) => (
          <div
            className={`px-3 py-2 text-sm rounded-md cursor-pointer ${isActive ? "bg-zinc-100 dark:bg-zinc-800" : "hover:bg-zinc-50 dark:hover:bg-zinc-900"}`}
          >
      ...
      {confirmDeleteId && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-2xl"
